...
def implied_vol(price: float, S: float, K: float, T: float, r: float, option: str) -> float:
    ...
    def objective(sigma: float) -> float:
        return bs_price(S, K, T, r, sigma, option) - price

    try:
        return newton(objective, x0=0.2, tol=1e-6, maxiter=100)
    except Exception:
        return np.nan
...
def compute_iv(
    ...
    df = df[df['Price'] > 0]
    df = df[df['S'] > 0]
    return df.groupby(['Strike', 'TTM'], as_index=False).mean(numeric_only=True)
...
def main():
    ...
    parser.add_argument("--rate", type=float, default=0.03, help="risk-free interest rate")
    parser.add_argument(
        "--sample",
        type=int,
        default=None,
    ...
    if args.compare:
        ...
        call_iv = compute_iv(call_df, 'c', r=args.rate, show_progress=args.verbose)
        call_iv['Option'] = 'Call'
        put_iv = compute_iv(put_df, 'p', r=args.rate, show_progress=args.verbose)
        put_iv['Option'] = 'Put'
        ...
        iv_df = compute_iv(df_opt, opt_type, r=args.rate, show_progress=args.verbose)
